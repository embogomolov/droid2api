...
(async()=>{
 ...
 let keys=ids.map((id,i)=>({id,key:'fk-never-render-me',notes:i===0?'Reserved':i===3?'New account':'Work '+i,poolGroup:'default',status:'active',excluded:i===0,last_test_result:i===3?'untested':'success',routing:{standard:{blocked:null},core:{blocked:null}},usage_count:12,last_used_at:Date.now()}));
 ...
 await page.route('**/*',async route=>{
  ...
  if(endpoint==='/window-sync'){
   ...
   return reply(route,{settings,state:{phase:'waiting',message:'Waiting for weekly quota',members:{},nextCheckAt:Date.now()+30000},running:true,error:null,timezone:'Europe/Moscow',keys:keys.map(k=>({...k,tested:k.last_test_result==='success'})),models:config.models.map(m=>({...m,group:'standard'}))});
  }
  if(endpoint==='/logs/history')return reply(route,{logs:[{type:'generation',timestamp:new Date().toISOString(),level:'info',summary:{model:'claude-haiku-4-5-20251001',keyId:ids[1],outcome:'Completed',elapsedMs:1600,usage:{input:1000,output:20,cacheRead:900}}},{type:'generation',level:'error',timestamp:new Date().toISOString(),summary:{model:'test-model',outcome:'HTTP 503',error:'<img src=x onerror=alert(1)> fk-hidden-secret',elapsedMs:100}}]});
 ...
 try{
  ...
  await page.locator('#hoursEnabled').check();await page.locator('#hoursStart').fill('10:15');
