...
function fixture() {
  ...
  let cfg = { window_sync: structuredClone(DEFAULT_WINDOW_SYNC), models: [{ id: DEFAULT_WINDOW_SYNC.groups.standard.modelId, type: 'anthropic' }] };
  ...
  const options = { manager: { keys }, directory, now: () => time, readConfig: () => cfg,
    ...
    send: async (key, model) => { assert.equal(JSON.parse(fs.readFileSync(path.join(directory, 'window_sync.json'))).groups.standard.members[key.id].attempts, 1, 'Persist before dispatch'); calls.push(key.id); windows[key.key].end = new Date(time + FIVE_HOURS).toISOString(); return { accepted: true, responseId: `fake-${key.id}` }; } };
...
try {
  ...
  f.cfg.window_sync.groups.standard.keyIds.push('c'); f.keys[2].excluded = true;
  ...
  assert.equal(restarted.load().groups.standard.phase, 'active'); assert.equal(restarted.load().groups.standard.spreadMs, 0);
  assert.deepEqual(f.calls, ['a','b'], 'Restart verifies without replay');
  assert.equal(restarted.routingBlock(f.keys[0], f.cfg.window_sync.groups.standard.modelId), null);
  f.advance(FIVE_HOURS); await restarted.tick(); assert.equal(f.calls.length, 4, 'Next full cycle starts automatically');

  const lost = fixture(); lost.cfg.window_sync.groups.standard.keyIds = ['a'];
  ...
  const retry = fixture(); retry.cfg.window_sync.groups.standard.keyIds = ['a'];
  ...
  const denied = fixture(); denied.cfg.window_sync.groups.standard.keyIds = ['a'];
  let sends = 0;
  ...
  const accepted = fixture(); accepted.cfg.window_sync.groups.standard.keyIds = ['a'];
  ...
  assert.equal(accepted.scheduler().load().groups.standard.phase, 'active');
  ...
  assert.equal(fs.existsSync(stoppingScheduler.lock), false); assert.equal(stoppingScheduler.load().groups.standard.phase, 'starting');
  ...
  exhausted.cfg.window_sync.groups.standard.keyIds = ['b'];
  ...
  assert.equal(exhausted.scheduler().routingBlock(exhausted.keys[0], exhausted.cfg.window_sync.groups.standard.modelId), null, 'Removed member is not held by synchronization');
  ...
  assert.equal(DEFAULT_WINDOW_SYNC.groups.standard.enabled, false); assert.equal(DEFAULT_WINDOW_SYNC.groups.standard.workingHours.enabled, false);
  ...
  const scheduled = fixture(); scheduled.cfg.window_sync.groups.standard.workingHours = { enabled: true, start: '00:00', end: '00:01' };
  await scheduled.scheduler().tick(); assert.equal(scheduled.calls.length, 0);
  scheduled.cfg.window_sync.groups.standard.workingHours.enabled = false; await scheduled.scheduler().tick(); assert.equal(scheduled.calls.length, 2);
  scheduled.cfg.window_sync.groups.standard.workingHours = { enabled: true, start: '09:00', end: '18:00' };
  ...
  assert.throws(() => validateWindowSync({ ...f.cfg.window_sync, groups:{...f.cfg.window_sync.groups,standard:{...f.cfg.window_sync.groups.standard,enabled:'true'}} }, f.keys, f.cfg.models), /boolean/);
  assert.throws(() => validateWindowSync({ ...f.cfg.window_sync, groups:{...f.cfg.window_sync.groups,standard:{...f.cfg.window_sync.groups.standard,keyIds:['missing']}} }, f.keys, f.cfg.models), /existing/);
  assert.throws(() => validateWindowSync({ ...f.cfg.window_sync, groups:{...f.cfg.window_sync.groups,standard:{...f.cfg.window_sync.groups.standard,keyIds:['a','a']}} }, f.keys, f.cfg.models), /duplicate/i);
